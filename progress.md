...
## 2026-09-11（目录分组 / 网段归类）

- 新增**目录**（`groups[]`）实体：侧栏可新建 `ATD`/`ADR` 等目录，网段通过 `groupId` 归属；目录标题可折叠/展开并记住状态，显示「N 网段 · M 已分配」；未归组的网段收在「未分组」区。
- schema 升到 v2，新增 `1 → 2` 迁移：补齐 `groups`/`groupId`，并清理指向已删目录的悬空归属。
- 删除目录只把其中网段置为未分组，**不删除网段与登记记录**（符合 PRODUCT.md 的破坏性操作要求）。
- 导入时「子网」列自动建同名目录并归类；同名目录不重复建；**已有归属的网段不被导入改写**，保护手工调整。
- 导出时「子网」列回填目录名（此前为留空的人工维护列）；12 列导出新增「目录」列。
- 修掉两处**测试脚手架**缺陷（非产品 bug）：selftest 里 stub 的 `Store.state` 缺 `ui`/`settings` 而 `Store.act` 会触发整段 `Render.all`；侧栏折叠断言在 `click()` 后仍持有旧元素引用，而 `Store.act` 会重建侧栏 DOM。
- 最终验证：内建 selftest `72/72`；真实 xlsx 端到端 `37/37`；UI DOM `29/29`（含 390px 无横向溢出）；边界 `21/21`；openpyxl 导出校验通过。
- 用户确认「目录即表格里的子网列」，撤除此前的「导出留空供人工维护」思路：`IO.HUMAN_COLUMNS` 重命名为 `IO.GROUP_COLUMNS`（表示由目录归属派生的列，而非人工列），导出恒由目录名回填；README/findings 措辞同步，明确两边是同一概念。
- 补充 selftest：子网列取值来自目录归属而非记录字段（记录里即使有 subnet 字段也以目录为准）；无「子网」列时网段留在未分组。selftest 增至 `74/74`。

## 2026-09-11（批量删除 / 滚动条 / 图标）

- 侧栏新增**多选模式**：点「多选」进入，点卡片勾选，**Shift 范围勾选**，目录头复选框**整目录全选**（部分选中显示 `–`），底部操作条显示「已选 N 个网段（M 条记录）」并提供 全选/取消/删除。
- 新增 `SideSel` 模块与 `subnet-delete-many` action：批量删除前确认框写明网段数与记录数；删除当前查看的网段会自动切到剩余第一个并关闭详情面板；单删时同步清理多选集合避免幽灵选中。
- 多选**不复用**格子图的 `Batch`：一个选网段、一个选 IP，语义不同，共用状态会让「删除」与「设状态」互相干扰。
- 修掉自查发现的问题：最初每次勾选都整段重建侧栏，导致连续点击的 DOM 引用失效（UI 测试实测两次点击只算一次）。改为 `SideSel.paint()` 只更新勾选态与计数，保留滚动位置与焦点。
- Shift 范围选择基于 `SideSel.visibleIds()`，**已折叠目录下的网段不参与**，与"所见即所选"一致。
- 侧栏与详情面板**永久隐藏滚动条**（`scrollbar-width:none` + `::-webkit-scrollbar`），滚轮/键盘滚动不受影响；已在 360px 高视口下断言滚动条厚度为 0 且 `scrollTop` 可动。
- 新增浏览器标签图标：因 CSP 为 `img-src data: blob:`，只能用**内联 SVG data URI**（外链 .ico 会被拦截）；两种方式验证——静态 SVG 解析 + 真实浏览器 `new Image()` 加载并监听 CSP 拦截日志。初次发现的"尺寸不符"实为未写 `width/height` 导致固有尺寸回落 150×150，补上后为 32×32。
- 最终验证：内建 selftest `80/80`；真实 xlsx 端到端 `37/37`；UI DOM `47/47`；边界 `21/21`；openpyxl 与 favicon 校验通过。
