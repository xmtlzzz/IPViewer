...
### P14 复查修复与增强（2026-09-11 第 8 轮）

- [x] **修复：撤销后多选悬空**——`Undo.restore` 恢复数据后过滤 `SideSel.sel` / `Batch.sel` 中已失效的 id（实测确认原本只影响计数显示，无数据破坏）
- [x] **修复：快照配额占用**——`pushSnapshot` 增加同内容去重（只更新时间戳）；数据 >1MB 时快照上限自动降为 3 份；写入失败先丢弃最旧一份重试
- [x] **修复：autoMap 缺标签规则**——表头含「标签/tag」现在能自动映射到 tags 字段
- [x] **修复：利用率预警缺失**——≥80% 黄色 / ≥95% 红色（带 ⚠），侧栏卡片、统计条、总览 Top5 三处同步
- [x] **增强：操作历史面板**——底部「历史」按钮（原「快照」）：上半为本次会话操作列表（操作名 + 时间 + CIDR 摘要），点「回到此前」跳回任意步且可用 Ctrl+Y 逐步重做回来；下半为跨会话持久快照。`Undo` 栈元素升级为 `{label, ts, snap}`，新增 `Store.jumpTo(i)`、`Undo.labelOf`
- [x] **增强：批量新增网段**——侧栏「批量」按钮：多行粘贴 CIDR（可带名称），自动规范化、跳过重复、无效行明细报告、可统一归入目录/VLAN
- [x] **修复：selftest 桩用例污染撤销历史**——自测结束后自动清空 `Undo` 栈（否则桩数据会出现在用户的操作历史里）
- [x] **侧栏拖拽调宽**：右边缘手柄拖动调整（200–560px，双击复位 260px），宽度持久化到 settings；拖动中实时预览、释放落盘、触摸屏支持；窄屏（<720px）自动隐藏手柄
- [x] **侧栏控件行自动换行**：控件按内容宽度排列，空间不足自动折行（不压缩截断）；340px 以上一行，窄于此时自动两行
- [x] 回归：selftest 100/100、P14 专项 17/17、历史面板专项 6/6、侧栏调宽专项 12/12、tools 三套件 37/47/21 全过，零运行时异常
- 开发期修复：`SideResize.init` 初版在 IIFE 顶层立即执行，早于 `Store.init()` 导致 `Store.state` 为 null 使整个脚本启动中断；已改为 `boot()` 内调用
