...
**预设格式自动识别**：表头含「设备名称 / 接口名称 / 接口IP / 掩码」时，自动识别为**华为设备 IP 统计表**并跳过手工映射，再由「接口IP + 掩码」推导所属网段（`192.168.156.9/24` → `192.168.156.0/24`）。此时「子网」列作为网段名称/分组使用。
...
**导出沿用导入格式**：导入后自动记住该文件的**表头与列序**，导出登记表时原样复现（换个顺序的模板导进来，导出去还是那个顺序）。其中「子网」等工具无法推导的列**保持为空，留给人工在 Excel 里维护**；模板里没有独立列的字段会汇总进备注列，不会丢信息。

**华为设备 IP 统计表格式**（`子网 / 设备名称 / 接口名称 / 接口IP / 掩码 / 备注`）：

| 列 | 处理方式 |
|---|---|
| 子网 | 导出留空，人工维护（导入时仅作网段命名参考） |
...
在地址栏后加 `?selftest` 打开自检页面（`index.html?selftest`），自动运行 67 条断言覆盖 CIDR 计算、掩码换算、CSV 解析、列映射、华为格式识别、导入/导出模板复现、zip/xlsx 编解码、NetBox IPAM/DCIM/元数据映射、实例隔离、分块和失败报告等核心逻辑。
